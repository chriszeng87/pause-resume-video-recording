...
{
    int i, j;
//TODO    setExitOnFail(pSession, ret);
    if (do_benchmark) {
                         ...
                         double sync_ipts)
{
    ...
    if (delta0 < 0 &&
        ...
        if (delta0 < -0.6) {
            av_log(NULL, AV_LOG_WARNING, "Past duration %f %f too large\n", -delta0, duration);
        } else
...
static void print_report(int is_last_report, int64_t timer_start, int64_t cur_time)
{
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (!vid && enc->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            snprintf(bufStat + strlen(bufStat), sizeof(bufStat) - strlen(bufStat), "%5d %3.*f",
    ...
    if (bitrate < 0) {
        ...
        snprintf(bufStat + strlen(bufStat), sizeof(bufStat) - strlen(bufStat), "\t%6.1fkbits/s", bitrate);
    }
...
//    updateStatBuf(pSession, bufStat, vfps, bitrate);
    if (progress_avio) {
...
#define CHECK_NULL_RETURN(PTR) if ( NULL == PTR ) { return 0; }
...
int ffmpeg_run( void * pSes, int argc, char** argv) {
